		...
		<?php if ( have_posts() ) while ( have_posts() ) : the_post(); ?>
			...
			<?php endif; ?>
				...
				if ( in_array( $mts_video_service, array( 'vimeo', 'dailymotion', 'facebook' ) ) ) {
					...
					$mts_show_player_related = isset( $mts_options['mts_player_related'] ) && $mts_options['mts_player_related'] ? '1' : '0';
					?>
					<script type="text/javascript">
						jQuery(document).ready(function() {
							jQuery('#myList').youtube_video({
								...
								<?php } ?>
									player_vars: {
										rel: <?php echo $mts_show_player_related; ?>,
									},
						...
						<div class="post-single-content box mark-links entry-content">
							...
							<div class="post-top">
								<?php if( !empty( $mts_options["mts_single_headline_meta_info"]['date']) && !empty($mts_options["mts_single_headline_meta"]) ) { ?>
									<div class="publish-date">
										<?php _e('Đăng lúc ','video'); the_time(get_option( 'date_format' )); ?>
									</div>
								<?php } ?>
								<?php if(has_post_format( 'video' )) { ?>
							...
							<div class="togglecontent">
								<div class="thecontent">
                                    ...
                                    <!-- sự kiện . Nếu bài viết trong thư mục Su-Kien thì kiểm tra field nếu có giá trị thì hiển thị...-->
                                    <?php if(in_category('su-kien')){?>
										<!-- hiện thị ảnh -->
										<div class="anh_su_kien">
											<?php the_post_thumbnail( 'medium_large' );  ?>
										</div>
                                        <div class="sukien">
											<?php  if( get_field('sukien_thoigian')) { ?>
												<span class='fa fa-clock-o'> Thời gian: <span class="sukien_thoigian"> <?php the_field('sukien_thoigian') ?> </span> &nbsp &nbsp &nbsp &nbsp &nbsp </span>
											
											<?php } ?>

											<?php  if( get_field('sukien_diadiem')) { ?>
												<span class='fa fa-map-marker'> Địa điểm: <span class="sukien_diadiem"> <?php the_field('sukien_diadiem') ?> </span></span>
												<?php } ?>

											<?php  if( get_field('sukien_diengia')) { ?>
												<span class='fa fa-microphone'> Diễn giả: <span class="sukien_diengia"> <?php the_field('sukien_diengia') ?> </span></span>
												<?php } ?>

											<?php  if( get_field('sukien_tochuc')) { ?>
												<span class='fa fa-trophy'> Tổ chức bởi: <span class="sukien_tochuc"> <?php the_field('sukien_tochuc') ?> </span></span>
												<?php } ?>
											<?php  if( get_field('sukien_link')) { ?>
												<span class='fa fa-link'> Link sự kiện: <a class="sukien_link" href="<?php the_field('sukien_link') ?>"> <?php the_field('sukien_link') ?> </a></span>
												<?php } ?>							
                                        </div>     
                                        <?php }?>
                                        <!-- đóng sự kiện -->

										<!-- việc làm . Nếu bài viết trong thư mục viec-lam thì kiểm tra field nếu có giá trị thì hiển thị...-->
										<?php if(in_category('viec-lam')){?>
										<div class="row vieclam">											
											<div class="col-xs-4 col-sm-4 col-md-4 col-lg-4">
												    <!-- hiện thị ảnh -->
													<div class="anh_viec_lam">
														<?php the_post_thumbnail( 'medium' );  ?>
													</div> 
											</div>
										
											<div class="col-xs-8 col-sm-8 col-md-8 col-lg-8">											
													<div class="vieclam_thongtin">
														<div class="row">															
															<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
																	<?php  if( get_field('vieclam_congty')) { ?>
																	<span class='fa fa-trophy'> <span class="vieclam_congty"> <?php the_field('vieclam_congty') ?> </span> </span>
														...
														<div class="row">															
															<div class="col-xs-6 col-sm-6 col-md-6 col-lg-6">
																	<?php  if( get_field('vieclam_luong')) { ?>
																	<span class='fa fa-money'> Lương: <span class="vieclam_luong"> <?php the_field('vieclam_luong') ?> </span></span>
																
																<?php } ?>
															</div>

															<div class="col-xs-6 col-sm-6 col-md-6 col-lg-6">
																<?php  if( get_field('vieclam_soluong')) { ?>
																<span class='fa fa-users'> Số lượng: <span class="vieclam_soluong"> <?php the_field('vieclam_soluong') ?> </span></span>
																<?php } ?>
															</div>															
														</div>
														...
														<div class="row">															
															<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
																	<?php  if( get_field('vieclam_diadiemlamviec')) { ?>
																	<span class='fa fa-map-marker'> Làm việc tại: <span class="vieclam_diadiemlamviec"> <?php the_field('vieclam_diadiemlamviec') ?> </span></span>
																	<?php } ?>

																	<?php  if( get_field('vieclam_hannophoso')) { ?>
																	<span class='fa fa-clock-o'> Hạn nộp hồ sơ: <span class="vieclam_hannophoso"> <?php the_field('vieclam_hannophoso') ?> </span></span>
																	<?php } ?>

																	<?php  if( get_field('vieclam_nophoso')) { ?>
																		<div> <?php the_field('vieclam_nophoso') ?></div>
																		<?php } ?>		
																	</div>
